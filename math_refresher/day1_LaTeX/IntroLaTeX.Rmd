...
- By now, I hope that you'll be able to tell what this code is doing. The only new thing here is `\includegraphics`, which is a command to include graphics(!). `[width=width-of-graphic]` specifies the width of the figure, where `\textwidth` is the width of the page (minus the margins); `{file-name}` specifies the name of the file that contains the figure
...
Below, I put everything together into one document. I've also added some `sections` and `subsections` to the document. The compiled pdf file can be found in my [github repository](https://github.com/baruuum/intro_to_stats_2019/tree/master/math_refresher/day1_Latex).
